...
export default function PlayersPage() {
  ...
  const fetchPlayerStats = async (playerId: number) => {
    ...
    const seasonsToTry = [2025, 2024, 2023]; // 尝试的年份
    let foundStats = null;
    ...
    for (const season of seasonsToTry) {
      try {
        ...
        if (data.data && data.data.length > 0) {
          foundStats = { ...data.data[0], displaySeason: `${season}-${season+1}` };
          break; // 只要查到一个年份有数据，就停止循环
        }
      } catch (err) {
        console.error(`查询 ${season} 失败`, err);
      }
    }
    ...
    <div className="min-h-screen bg-[#0b0e11] text-white p-6 md:p-12 relative font-sans text-sm">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <form onSubmit={(e) => { e.preventDefault(); setPage(1); fetchPlayers(search, 1); }} className="flex gap-3 mb-12">
          <input 
            type="text"
            placeholder="搜索球员 (如: James, Curry, Jokic)"
          ...
          />
          <button type="submit" className="bg-blue-600 px-10 py-5 rounded-3xl font-black uppercase tracking-widest hover:bg-blue-700 shadow-lg">Search</button>
        </form>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {players.map((p: any) => (
            <div key={p.id} onClick={() => handlePlayerClick(p)} className="bg-[#16191d] border border-zinc-800 p-6 rounded-3xl hover:border-blue-500 transition-all cursor-pointer group">
              ...
              <h3 className="text-xl font-black group-hover:text-blue-400 uppercase italic leading-tight">{p.first_name} {p.last_name}</h3>
        ...
        {players.length > 0 && (
          <div className="mt-16 text-center pb-20">
            <button onClick={() => { const next = page + 1; setPage(next); fetchPlayers(search, next); }} disabled={loading} className="bg-zinc-900 border border-zinc-800 px-12 py-4 rounded-full text-xs font-black uppercase">
            ...
            <div className="p-8">
              ...
              <div className="bg-zinc-900/30 border border-zinc-800 rounded-[2rem] p-8">
                <div className="flex justify-between items-center mb-8">
                  <h4 className="text-xs font-black text-zinc-400 uppercase tracking-[0.3em]">{playerStats?.displaySeason || 'SEASON'} AVERAGES</h4>
                  <span className="text-[10px] bg-blue-500/10 text-blue-500 px-3 py-1 rounded-full font-bold italic uppercase">Live Stats</span>
                </div>
                ...
                {loadingStats ? (
                  ...
                  <div className="space-y-8 text-center">
                    <div className="grid grid-cols-3 gap-4">
                      <div><p className="text-5xl font-black italic tracking-tighter">{playerStats.pts}</p><p className="text-[10px] text-blue-500 font-bold uppercase mt-2">Points</p></div>
                      <div className="border-x border-zinc-800 px-4"><p className="text-5xl font-black italic tracking-tighter">{playerStats.reb}</p><p className="text-[10px] text-blue-500 font-bold uppercase mt-2">Rebounds</p></div>
                      <div><p className="text-5xl font-black italic tracking-tighter">{playerStats.ast}</p><p className="text-[10px] text-blue-500 font-bold uppercase mt-2">Assists</p></div>
                    </div>
                  ...
                  <div className="text-center py-12 text-zinc-600 italic border border-dashed border-zinc-800 rounded-3xl">
                    No stats available for recent seasons.
                  </div>
                )}
              </div>
              <button onClick={() => setSelectedPlayer(null)} className="w-full mt-8 bg-zinc-800 text-white py-5 rounded-[1.5rem] font-black uppercase tracking-[0.2em] text-xs">Close Terminal</button>
            </div>
          </div>
        </div>
      )}
