    ...
    <div className="min-h-screen bg-[#0b0e11] text-white p-6 md:p-12 relative font-sans">
      ...
      <div className="max-w-7xl mx-auto">
        <form onSubmit={(e) => { e.preventDefault(); setPage(1); fetchPlayers(search, 1); }} className="flex gap-3 mb-12">
          <input 
            type="text"
            placeholder="搜索球员 (如: Curry, James, Durant)"
            className="flex-1 bg-[#16191d] border border-zinc-800 p-5 rounded-3xl outline-none text-white text-lg"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
          />
          <button type="submit" className="bg-blue-600 px-10 py-5 rounded-3xl font-black uppercase tracking-widest hover:bg-blue-700 transition-all">SEARCH</button>
        </form>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {players.map((p: any) => (
            <div key={p.id} onClick={() => { setSelectedPlayer(p); fetchPlayerStats(p.id); }} className="bg-[#16191d] border border-zinc-800 p-6 rounded-3xl hover:border-blue-500 transition-all cursor-pointer group">
            ...
            <div className="p-8">
              <div className="grid grid-cols-3 gap-4 mb-8">
                ...
                <div className="bg-zinc-900/50 p-4 rounded-2xl border border-zinc-800 text-center">
                  ...
                  <p className="text-lg font-black text-white">{selectedPlayer.height_feet ? `${selectedPlayer.height_feet}'${selectedPlayer.height_inches}"` : '--'}</p>
                </div>
                <div className="bg-zinc-900/50 p-4 rounded-2xl border border-zinc-800 text-center">
                  <p className="text-zinc-500 text-[10px] font-bold uppercase mb-1 tracking-widest">Weight</p>
                  <p className="text-lg font-black text-white">{selectedPlayer.weight_pounds ? `${selectedPlayer.weight_pounds} lbs` : '--'}</p>
                </div>
              </div>
              ...
              <div className="bg-zinc-900/30 border border-zinc-800 rounded-[2rem] p-8">
                <div className="flex justify-between items-center mb-8">
                  ...
                  <span className="text-[10px] bg-blue-500/10 text-blue-500 px-3 py-1 rounded-full font-bold italic">FREE TIER API</span>
                </div>
                ...
                {loadingStats ? (
                  ...
                  <div className="space-y-8 text-center">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6 pt-8 border-t border-zinc-800/50">
                      <div><p className="text-zinc-500 text-[10px] font-bold uppercase mb-1 tracking-widest text-[9px]">FG%</p><p className="text-xl font-bold">{formatPct(playerStats.fg_pct)}</p></div>
                      <div><p className="text-zinc-500 text-[10px] font-bold uppercase mb-1 tracking-widest text-[9px]">3P%</p><p className="text-xl font-bold">{formatPct(playerStats.fg3_pct)}</p></div>
                      <div><p className="text-zinc-500 text-[10px] font-bold uppercase mb-1 tracking-widest text-[9px]">GP</p><p className="text-xl font-bold">{playerStats.games_played}</p></div>
                      <div><p className="text-zinc-500 text-[10px] font-bold uppercase mb-1 tracking-widest text-[9px]">MIN</p><p className="text-xl font-bold">{playerStats.min}</p></div>
                    </div>
                  </div>
                ) : (
                  <div className="text-center py-12 border border-dashed border-zinc-800 rounded-3xl">
                    <p className="text-zinc-500 text-sm font-medium">数据暂未由 API 录入</p>
                    <p className="text-zinc-700 text-[10px] mt-2 uppercase tracking-widest">Free data source has limited coverage</p>
                  </div>
                )}
              </div>
              <button onClick={() => setSelectedPlayer(null)} className="w-full mt-8 bg-zinc-800 text-white py-5 rounded-[1.5rem] font-black uppercase tracking-[0.2em] text-xs transition-colors hover:bg-zinc-700">CLOSE DATA TERMINAL</button>
            </div>
          </div>
        </div>
      )}
