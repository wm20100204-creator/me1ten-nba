  ...
  const fetchPlayersTerminal = async (nameToSearch: string) => {
    ...
    try {
      ...
      if (data.data && data.data.length > 0) {
        ...
        setErrorMsg("球员库未检索到该姓名。建议尝试搜索姓氏。");
      }
    } catch (e) { setErrorMsg("数据终端同步失败。"); }
    setLoading(false);
  };
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="flex gap-4 mb-12 px-4 md:px-0">
        <input 
          ...
          className="flex-1 bg-[#16191d] border border-zinc-800 p-5 rounded-[2rem] outline-none text-white text-lg focus:border-blue-500 shadow-2xl transition-all"
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          onKeyDown={(e) => e.key === 'Enter' && fetchPlayersTerminal(search)}
        />
        <button onClick={() => fetchPlayersTerminal(search)} className="bg-blue-600 px-10 py-5 rounded-[2rem] font-black uppercase tracking-widest hover:bg-blue-700 shadow-xl transition-all active:scale-95">
          Search
        </button>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-3xl bg-black/95">
          ...
          <div className="bg-[#111317] border border-zinc-800 w-full max-w-5xl max-h-[85vh] overflow-y-auto rounded-[3rem] shadow-2xl relative">
            ...
            <div className="bg-blue-600 p-6 md:p-10 flex flex-col md:flex-row items-center gap-8 relative overflow-hidden border-b border-white/10 sticky top-0 z-30">
              ...
              <div className="w-32 h-32 md:w-44 md:h-44 bg-zinc-900 rounded-full flex-shrink-0 border-4 border-white/20 relative z-10 overflow-hidden shadow-2xl">
              ...
              <div className="text-center md:text-left z-10">
                ...
                <div className="flex flex-wrap justify-center md:justify-start items-center gap-3">
                   <div className="flex items-center gap-2 bg-black/20 px-4 py-1.5 rounded-full border border-white/10">
                      <img src={`https://a.espncdn.com/i/teamlogos/nba/500/scoreboard/${getEspnAbbr(selectedPlayer.team?.abbreviation)}.png`} className="w-6 h-6 object-contain" />
                   ...
                   {PLAYER_AWARDS[`${selectedPlayer.first_name} ${selectedPlayer.last_name}`] && (
                     <span className={`${PLAYER_AWARDS[`${selectedPlayer.first_name} ${selectedPlayer.last_name}`].color} text-black px-3 py-1 rounded-full text-[9px] font-black italic shadow-lg`}>
            ...
            <div className="p-8 md:p-12 space-y-10">
              ...
              <div className="grid grid-cols-3 gap-6">
                ...
                <div className="bg-zinc-900/50 p-6 rounded-3xl border border-zinc-800 text-center shadow-inner">
                  ...
                  <p className="text-2xl font-black text-white italic">{selectedPlayer.weight || '--'} <span className="text-xs">lbs</span></p>
                </div>
                <div className="bg-zinc-900/50 p-6 rounded-3xl border border-zinc-800 text-center shadow-inner">
              ...
              <div className="bg-[#16191d] border border-zinc-800 rounded-[2.5rem] p-10 shadow-inner">
                <h4 className="text-[10px] font-black text-zinc-600 uppercase tracking-[0.5em] italic mb-10 text-center">Identity Intelligence Terminal</h4>
                
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 text-left font-black italic">
                   <div className="border-l-2 border-zinc-800 pl-4">
                      <p className="text-[8px] text-zinc-500 uppercase tracking-widest mb-1">NBA Draft Info</p>
                   ...
                   <div className="border-l-2 border-zinc-800 pl-4">
                      ...
                      <p className="text-base text-white uppercase leading-tight truncate">{selectedPlayer.college || 'Professional'}</p>
                   </div>
                   <div className="border-l-2 border-zinc-800 pl-4">
                      <p className="text-[8px] text-zinc-500 uppercase tracking-widest mb-1">Country</p>
                      <p className="text-base text-white uppercase leading-tight">{selectedPlayer.country || 'USA'}</p>
                   </div>
                   <div className="border-l-2 border-zinc-800 pl-4">
                      <p className="text-[8px] text-zinc-500 uppercase tracking-widest mb-1">Status</p>
                      <p className="text-base text-green-500 uppercase leading-tight tracking-widest animate-pulse">Active Node</p>
                   </div>
                </div>
              </div>
              ...
              <button 
                ...
                className="w-full bg-zinc-800 py-6 rounded-3xl font-black uppercase tracking-widest text-[10px] hover:bg-white hover:text-black transition-all border border-zinc-700/50"
              >
