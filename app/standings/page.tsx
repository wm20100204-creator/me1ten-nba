...
export default function StandingsPage() {
  ...
  // 【核心修复】：通过 2025 赛季的比赛统计来反推“现役名单”
  const fetchActiveRoster = async (teamId: number) => {
    setLoadingRoster(true);
    setRoster([]);
    try {
      // 逻辑：查询该队在 2025 赛季所有球员的比赛统计
      const res = await fetch(`https://api.balldontlie.io/v1/stats?team_ids[]=${teamId}&seasons[]=2025&per_page=100`, { headers: PRO_HEADERS });
      const data = await res.json();
      
      // 从比赛数据中提取球员并去重
      const statsEntries = data.data || [];
      ...
      // 转换为数组并排序
      const activePlayers = Array.from(playerMap.values()).sort((a, b) => a.last_name.localeCompare(b.last_name));
      
      setRoster(activePlayers);
    } catch (e) {
      console.error("名单抓取失败", e);
    } finally {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-3xl bg-black/95">
          <div className="bg-[#111317] border border-zinc-800 w-full max-w-4xl max-h-[90vh] rounded-[3rem] overflow-y-auto shadow-2xl relative animate-in slide-in-from-bottom-10 duration-500">
            ...
            <div className="sticky top-0 z-20 bg-zinc-900/95 backdrop-blur-md p-8 border-b border-white/10 flex justify-between items-center">
               <div className="flex items-center gap-6">
                  ...
                  <div>
                    ...
                    <p className="text-blue-500 font-bold uppercase tracking-[0.2em] text-[10px] mt-2">Verified Active Roster • 2025-26</p>
                  </div>
               </div>
            ...
            <div className="p-10 space-y-12">
              ...
              <div className="space-y-8">
                <h4 className="text-[10px] font-black text-zinc-500 uppercase tracking-widest border-l-2 border-red-600 pl-4 flex justify-between items-center text-white">
                  ...
                  <span className="text-[8px] bg-red-600/20 text-red-500 px-2 py-0.5 rounded font-black uppercase">Live Stats Filter On</span>
                </h4>
                ...
                {loadingRoster ? (
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {roster.length > 0 ? roster.map((player: any) => (
                      <div key={player.id} className="bg-zinc-900/50 p-4 rounded-2xl border border-zinc-800 hover:border-blue-500 transition-all group overflow-hidden relative">
            ...
            <div className="p-8 pt-0">
               <button onClick={() => setSelectedTeam(null)} className="w-full bg-zinc-800 py-6 rounded-3xl font-black uppercase tracking-widest text-[10px] hover:bg-zinc-700 transition-all">Close Team Terminal</button>
            </div>
          </div>
        </div>
      )}
