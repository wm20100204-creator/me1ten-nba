  ...
  const TeamCard = ({ t }: any) => (
    <div className="flex items-center justify-between p-6 bg-[#16191d] border border-zinc-800 rounded-[2.5rem] hover:border-blue-500 transition-all shadow-xl group overflow-hidden relative">
      <div className="flex items-center gap-6 relative z-10">
        {/* 使用最稳定的官方 SVG 路径 */}
        <div className="w-20 h-20 flex items-center justify-center bg-zinc-900 rounded-3xl p-3 border border-zinc-800/50">
            <img 
               src={`https://cdn.nba.com/logos/nba/${t.id}/primary/L/logo.svg`} 
        ...
        <div>
          ...
          <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest mt-2">{t.city} | {t.division}</p>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#0b0e11] text-white p-6 md:p-12 font-sans">
      <nav className="max-w-7xl mx-auto flex justify-between items-center mb-20 border-b border-zinc-800 pb-10">
        ...
        <Link href="/" className="text-zinc-500 font-bold text-[10px] hover:text-white uppercase tracking-[0.3em] border border-zinc-800 px-8 py-3 rounded-full transition-colors">Back to Home</Link>
      </nav>

      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16">
        <section>
          <h2 className="text-3xl font-black italic text-blue-500 mb-10 uppercase tracking-[0.3em] flex items-center gap-4">
        ...
        <section>
          <h2 className="text-3xl font-black italic text-red-600 mb-10 uppercase tracking-[0.3em] flex items-center gap-4">
